...
namespace Helper {
	...
	void displayErrorWithPort(size_t port, fmt::format_string<Args...> fmt_string, Args &&... args) {
		fmt::print(fmt::fg(fmt::color::red), "Server Error (port {}): ",port);
		fmt::print("{}\n\n", fmt::format(fmt_string, std::forward<Args>(args)...));
	}

	template<typename... Args>
	void displayMessageWithPort(size_t port, fmt::format_string<Args...> fmt_str, Args &&... args) {
		fmt::print(fmt::fg(fmt::color::green), "Server Info (port {}): ",port);
